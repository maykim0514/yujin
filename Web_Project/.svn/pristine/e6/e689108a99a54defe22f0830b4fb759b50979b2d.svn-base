...
<body class="is-preload">
	...
	<div id="main">
		...
		<div id="radio">
			<div>
				<p>
					<input type="radio" id="salecheck" name="customercheck" onclick="window.location.href='${pageContext.request.contextPath}/board/sale_list.bo?category=all'" checked/>
							<label for="salecheck" style="padding:0">구매</label><br>
					<input type="radio" id="rentcheck" name="customercheck" onclick="window.location.href='${pageContext.request.contextPath}/board/rent_list.bo?category=all'"/>
				...
				<hr>
				<ul>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=여성의류">여성의류</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=남성의류">남성의류</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=패션잡화">패션잡화</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=디지털/가전">디지털 / 가전</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=도서/티켓/취미/애완">도서 / 티켓 / 취미 / 애완</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=생활/문구/가구">생활 / 문구 / 가구</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=스포츠/레저">스포츠 / 레저</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=뷰티/미용">뷰티 / 미용</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=유아동/출산">유아동 / 출산</a></li>
					<li><a href="${pageContext.request.contextPath}/board/sale_list.bo?category=기타">기타</a></li>	
				</ul>
			</div>
		</div>
	</div>
	...
	<div class="wrap">
		<div class="inner">
			...
			<!-- radio 선택시 display 변화 있음 -->		
	 		<div id="purchase_list">
				<div class="list_name" style="display:inline;">나와 가까운 상품</div>
						<c:choose>
							<c:when test="${product_list_location!=null and fn:length(product_list_location)>0}">
								<ul class="nearby_list">
									<c:forEach var="i" begin="0" end="${fn:length(product_list_location)-1}">
										<c:forEach var="j" begin="0" end="${fn:length(product_list_location)-1}">
											<li>
												<c:if test="${product_list[j].getBoardnum() == product_list_location[i].getBoardnum()}">
													<dl>
														<dd>
															<a href="${pageContext.request.contextPath}/productlist/product.pr?num=${product_list_location[i].getBoardnum()}">
															<c:choose>
																<c:when test="${product_list[j].getFilename() != 'null'}">
								...
								<ul class="product_list">
									<c:forEach var="i" begin="0" end="${fn:length(product_list)-1}">
										<li>
											<dl>
												<dd>
													<a id="listdetail" href="${pageContext.request.contextPath}/board/BoardDetail.bo?num=${product_list[i].getBoardnum()}">
														<c:choose>
															<c:when test="${product_list[i].getFilename() != 'null'}">
																<img src="${pageContext.request.contextPath}/sale,rentalpost/files/${product_list[i].getFilename()}" width="194" height="194" alt="상품 이미지">
															</c:when>
															<c:otherwise>
												...
												<dd class="" id="product_name">
													<a href="${pageContext.request.contextPath}/board/BoardDetail.bo?num=${product_list[i].getBoardnum()}">${product_list[i].getBoardTitle()}</a>
												</dd>
												<dd class="" id="price"
													style="font-weight: bold; font-size: 20px">
													<a href="${pageContext.request.contextPath}/board/BoardDetail.bo?num=${product_list[i].getBoardnum()}">${product_list[i].getProductPrice()}</a>
												</dd>
												<dd class="" id="location">
													<a href="${pageContext.request.contextPath}/board/BoardDetail.bo?num=${product_list[i].getBoardnum()}" style="font-size: 15px">${product_list[i].getLocation()}</a>
												</dd>
											</dl>
										</li>
									</c:forEach>

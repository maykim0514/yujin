...
<html>
	<head>
		...
		<link rel="stylesheet" href="${pageContext.request.contextPath}/adminpage/assets/css/mainfixed.css" />
...
<!-- css 끝 -->
		<script src="../adminpage/jquery-3.5.1.js" type="text/javascript" text="javascript"></script>
		<script src="../adminpage/mixitup.min.js" type="text/javascript" text="javascript"></script>
<!-- 자바스크립트 시작 -->
		<script>
	//믹스
			$(function(){
...
<!-- HTML시작 -->
	...
	<body class="is-preload">
		...
		<c:set var="reportList" value="${requestScope.reportArray}" />
	...
	<!-- 신고 박스 -->
		...
		<!-- 미처리 -->
				...
				<!-- 신고 예시 -->
						<c:choose>
							<c:when test="${reportList eq null}">
								<div style="text-align:center">
									접수된 신고내용이 없습니다.
								</div>
							</c:when>
							<c:otherwise>
								<c:forEach var="i" begin="0" end="${fn:length(reportList)-1}">
									<div class="mix apple profile fraud info ad" data-order="1">
										<div class="rep_d_cnt">
											<p>${reportList[i].getReportdate()}</p>
										...
										<div style="height:85px;font-size:10px;width:90%;">
											...
											<div style="font-size:16px;float:left;line-height:60px;">${reportList[i].getUserid()}</div>
										...
										<div class="field" style="width:90%;margin:0 auto; margin-top:-10px;">
											<textarea name="message" id="message" rows="4" style="font-size:14px;" readonly>${reportList[i].getReport()}</textarea>
										</div>
...
<!-- Scripts -->
			...
			<script src="../adminpage/assets/js/jquery.dropotron.min.js"></script>
			<script src="../adminpage/assets/js/browser.min.js"></script>
			<script src="../adminpage/assets/js/breakpoints.min.js"></script>
			<script src="../adminpage/assets/js/util.js"></script>

...
<style>
	*:focus { 
	...
	/* .checkbox-control:hover .checkbox-content {
		display: block;
	}
	.checkbox-control:hover .checkboxbtn {
...
<body class="is-preload">
	...
	<div id ="wrap">
			...
			<div>
				<div style="margin-left:17%">
					<a href="${pageContext.request.contextPath}/mail/MailWrite.ma" class="button primary small"
						onClick="window.open(this.href,'_blank','width=600px, height=630px,left=1200px,top=500px');return false;">쪽지 보내기
					</a><br><br>		
				</div>
				<div class="inboxlocation" align="center">
					<a href="#" class="button primary small" style="background-color : #F59A23 !important">받은 쪽지함</a><br>
					<a href="${pageContext.request.contextPath}/mail/SentMailList.ma" class="button primary small">보낸 쪽지함</a><br>
		...
		<section id="section">
			...
			<div id="table-header" style="padding:0.8rem;height: 84px;border-bottom: 1px solid #ddd;">
				<div class="checkbox-dropdown">
					...
					<div class="checkbox-content" id="listDropdown">
						...
						<a href="#" onclick="getList(1, 'inboxRead')">읽음</a>
						<a href="#" onclick="getList(1, 'inboxUnread')">읽지않음</a>
					</div>
					...
					<div class="toggle-btn" style="display:none;">
						<a href="#" onclick="" class="button small" id="small">삭제</a>
					</div>
				</div>
	...
	<script>
			...
			$.ajax({
				...
				success:function(list){
					...
					if(list.inboxList == null || list.inboxList.length == 0){
						var str = "<td colspan='5' align='center'>받은 쪽지가 없습니다.</td>"
						mailUL.html(str);
						return;
					}
					...
					for(let i=0, len=list.inboxList.length; i < len; i++){
						if(list.inboxList[i].readstate == 1){	
							str2 += "<tr id='tdWrap' style='background : #fafafa;'><td style='vertical-align: top;'>"
						}else{
							str2 += "<tr id='tdWrap'><td style='vertical-align: top;'>"
						}
						...
						if(list.inboxList[i].important != "1"){
							...
							str2 += "<td>" + list.inboxList[i].userid + "</a></td>"
							str2 += "<td><a id='getDetail' href='javascript:getMailDetail(" + 
									list.inboxList[i].mailnum + ");' class='content'>" + 
		...
		function getMailDetail(mailnum){
			console.log(mailnum + "번 쪽지의 getMailDetail!")
			...
			$.ajax({
				...
				success:function(detail){
					...
					str += "<a href='${pageContext.request.contextPath}/mail/Reply.ma?mailnum=" + detail.mailnum + "' class='button small' id='small' name='sendMail'>답장</a>"
					...
					str += "받는 사람&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + detail.targetid + "<br>"
					str += "받은시간&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + detail.maildate + "</div></div>"
					str += "<div style='margin: 0 15px; width: 70%'><br>"
					str += detail.mailcontent + "<br><br><br><br><br><br><br><br><br><br><br>"
					str += "<div style='text-align:center; margin-left:27%'>"
					str += "<a href='javascript:changeMail(" + detail.mailnum + ", 0)' id='prevMail' style='color:black;font-size:0.85em'>이전</a> |"
					str += "<a href='javascript:changeMail(" + detail.mailnum + ", 1)' id='nextMail' style='color:black;font-size:0.85em'>다음</a></div></div>"
...
//쪽지함----------------------------------------------------------------------------------------------------------------------		
		...
		//쪽지보내기 작은창으로 띄우기 (이건 보낸 쪽지함에서만 사용)
		$(".openMail").on("click", "a[name='sendMail']", function(){
			window.open(this.href,'_blank','width=600px, height=630px,left=1200px,top=500px');
			return false;
		});
		...
		$(".paging").on("click", "a#changePage", function(e){
			e.preventDefault();
			console.log("페이징안 : " + keyword + "-" + option)
			getList($(this).attr('href'), "inbox", keyword, option);
		});
	...
	//쪽지 상세보기 ------------------------------------------------------------------------------------------------------------------
		...
		//쪽지 상세보기 페이징 처리
		function changeMail(mailnum, num, keyword, option){
			console.log("changeMail 들어옴")
			$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath}/mail/ChangeMail.ma?mailnum=" + mailnum + "&action=" + num + "&mailtype=inbox"
						+ "&keyword=" + keyword + "&option=" + option,
				success:function(result){
					result = JSON.parse(result);
		...
		$(".openMail").on("click", "a[name='goBack']", function(e){
			...
			$(".table-wrapper").show();
			$(".search-wrap").show();
			getList(pageNum, "inbox", keyword, option);
		})
...
//검색하기--------------------------------------------------------------------------------------------------------
		...
		$(".searchButton").on('click', function(e){
				...
				getList(1, "inbox", keyword, option)
			}
		);
	</script>
</body>

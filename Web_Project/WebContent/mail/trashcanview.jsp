...
<body class="is-preload">
		...
		<aside id="aside">
			...
			<div>
				...
				<div class="inboxlocation" align="center">
					...
					<a href="#" class="button primary small" style="background-color : #F59A23 !important">휴지통</a><br>
				</div>
			</div>
		</aside>
			...
			<div id="table-header" style="padding:0.8rem; height:84px;">
				<div class="checkbox-dropdown">
					...
					<div class="checkbox-content" id="listDropdown">
						...
						<a href="#" onclick="getList(1, 'trashRead')">읽음</a>
						<a href="#" onclick="getList(1, 'trashUnread')">읽지않음</a>
					</div>
					...
					<div class="toggle-btn" style="display:none;">
						<a href="#" onclick="selectAction('restore')" class="button small" id="small">복구</a>
						<a href="#" onclick="selectAction('empty')" class="button small" id="small">비우기</a>
					</div>
				</div>
	...
	<script>
			...
			$.ajax({
				...
				success:function(list){
					...
					if(list.inboxList == null || list.inboxList.length == 0){
						var str = "<td colspan='5' align='center'>휴지통이 비어있습니다.</td>"
						mailUL.html(str);
						return;
					}
					...
					for(let i=0, len=list.inboxList.length; i < len; i++){
						if(list.inboxList[i].readstate == 1 || list.inboxList[i].userid == "${session_id.getUserid()}"){	
						...
						if(list.inboxList[i].userid == "${session_id.getUserid()}"){
							/* 내가 보낸 쪽지 */
		...
		function getMailDetail(mailnum){
			console.log(mailnum + " getMailDetail!")
			...
			$.ajax({
				...
				success:function(detail){
					...
					if(detail.userid != "${session_id.getUserid()}"){
						/* 상대방한테 받은 쪽지 */
...
//쪽지함----------------------------------------------------------------------------------------------------------------------
		...
		$(".paging").on("click", "a#changePage", function(e){
			e.preventDefault();
			getList($(this).attr('href'), "trash", keyword, option);
		});
		...
		function selectAction(action){
			...
			$.ajax({
				type: "post",
				url: "${pageContext.request.contextPath}/mail/TrashDecision.ma",
...
//쪽지 상세보기 ------------------------------------------------------------------------------------------------------------------
		...
		function changeMail(mailnum, num){
			$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath}/mail/ChangeMail.ma?mailnum=" + mailnum + "&action=" + num + "&mailtype=trash",
		...
		$(".openMail").on("click", "a[name='goBack']", function(e){
			...
			$(".table-wrapper").show();
			$(".search-wrap").show();
			getList(pageNum, "trash", keyword, option);
		})
...
//검색하기--------------------------------------------------------------------------------------------------------
		...
		$(".searchButton").on('click', function(e){
				...
				getList(1, "trash", keyword, option)
			}
		);
	</script>		
	</body>

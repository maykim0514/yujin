...
<body class="is-preload">
	...
	<div id ="wrap">
		...
		<aside>
			...
			<div>
				...
				<div class="inboxlocation" align="center">
					...
					<a href="#" class="button primary small" style="background-color : #F59A23 !important">임시 쪽지함</a><br>
		...
		<section id="section">
			...
			<div style="padding:0.8rem; height:84px;">
	...
	<script>
		...
		//draftinboxview.jsp 들어오자마자 임시 쪽지함 목록 보여주기
		getList(1, "draft");
			...
			$.ajax({
				...
				success:function(list){
					...
					if(list.inboxList == null || list.inboxList.length == 0){
						var str = "<td colspan='5' align='center'>임시 저장한 쪽지가 없습니다.</td>"
						mailUL.html(str);
						return;
					}
					...
					for(let i=0, len=list.inboxList.length; i < len; i++){
						...
						str2 += "<td>" + list.inboxList[i].targetid + "</td>"
						str2 += "<td><a id='sendMail' href='${pageContext.request.contextPath}/mail/DraftWrite.ma?mailnum=" + 
								list.inboxList[i].mailnum + "' class='content'>" + list.inboxList[i].mailcontent + "</a></td>"
...
//쪽지함-----------------------------------------------------------------------------------------------
		...
		$(".paging").on("click", "a#changePage", function(e){
			e.preventDefault();
			getList($(this).attr('href'), "draft", keyword, option);
		});
		
		//쪽지보내기 작은창으로 띄우기
		$(".mailList").on("click", "a#sendMail", function(){
...
//검색하기--------------------------------------------------------------------------------------------
		...
		$(".searchButton").on('click', function(e){
				...
				getList(1, "draft", keyword, option)
			}
		);
	</script>	

		...
		<aside id="aside">
			...
			<div>
				...
				<div class="inboxlocation" align="center">
					...
					<a href="#" class="button primary small" style="background-color : #F59A23 !important">별표 쪽지함</a><br>
			...
			<div id="table-header" style="padding:0.8rem; height:84px;">
				<div class="checkbox-dropdown">
					...
					<div class="checkbox-content" id="listDropdown">
						...
						<a href="#" onclick="getList(1, 'impRead')">읽음</a>
						<a href="#" onclick="getList(1, 'impUnread')">읽지않음</a>
					</div>
			...
			$.ajax({
				...
				success:function(list){
					...
					if(list.inboxList == null || list.inboxList.length == 0){
						var str = "<td colspan='5' align='center'>중요 쪽지가 없습니다.</td>"
						mailUL.html(str);
						return;
					}
...
//쪽지함----------------------------------------------------------------------------------------------------------------------
		...
		$(".paging").on("click", "a#changePage", function(e){
			e.preventDefault();
			getList($(this).attr('href'), "important", keyword, option);
		});
...
//쪽지 상세보기 ------------------------------------------------------------------------------------------------------------------
		...
		function changeMail(mailnum, num){
			$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath}/mail/ChangeMail.ma?mailnum=" + mailnum + "&action=" + num + "&mailtype=important",
				success:function(result){
					result = JSON.parse(result);
		...
		$(".openMail").on("click", "a[name='goBack']", function(e){
			...
			$(".table-wrapper").show();
			$(".search-wrap").show();
			getList(pageNum, "important", keyword, option);
		})
...
//검색하기--------------------------------------------------------------------------------------------------------		
		...
		$(".searchButton").on('click', function(e){
				...
				getList(1, "important", keyword, option)
			}
		);
	</script>		
	</body>

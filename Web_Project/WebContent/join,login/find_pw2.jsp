...
<%-- <h3>${userid}</h3>  잘 받아옴--%>
	...
	<div id="container" class="inner">
		...
		<section id="findcontent">
			<span class="step" style="margin-left: 9.5%;">
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
				if (xhr.responseText.trim() == "ok") {
					...
					location.href = "${pageContext.request.contextPath}/member/find_id_phone.me?username="+$("input[name=username1]").val()+"&userphone="+$("#userphone").val()+"&page=find_pw2";
				} else {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
				if (xhr.responseText.trim() == "ok") {
					...
					location.href = "${pageContext.request.contextPath}/member/find_id_email.me?username="+$("input[name=username2]").val()+"&useremail="+$("#useremail").val()+"&page=find_pw2";;
				} else {
					alert("인증번호가 일치하지 않습니다.");
				}
			}
